...
class Dis_ContentOcclusion(nn.Module):
  def __init__(self):
    super(Dis_ContentOcclusion, self).__init__()
    model = []
    model += [LeakyReLUConv2d(256*2, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
    model += [LeakyReLUConv2d(256, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
    model += [LeakyReLUConv2d(256, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
    model += [LeakyReLUConv2d(256, 256, kernel_size=4, stride=1, padding=0)]
...
class MD_E_content(nn.Module):
  def __init__(self, input_dim, focus_layer):
    ...
    tch *= 2
    enc_c += [ReLUINSConv2d(tch, tch * 2 + focus_layer, kernel_size=3, stride=2, padding=1)]
    tch *= 2
    tch += focus_layer
    for i in range(0, 3):
  ...
  def forward(self, xa, xb):
    return self.conv(xa), self.conv(xb)
...
class MD_E_attr(nn.Module):
  ...
  def forward(self, x, c=torch.zeros(0)):
    if c.size(0) != 0:
...
class MD_G_multi_concatEo(nn.Module):
  def __init__(self, output_dim, c_dim=3, nz=8):
    super(MD_G_multi_concatEo, self).__init__()
    self.nz = nz
    ini_tch = 256
    tch_add = ini_tch
    tch = ini_tch

    self.decFusion = ReLUINSConv2d(tch * 2, tch, kernel_size=3, stride=1, padding=1)
  ...
  def forward(self, x, z, c):
    ...
    z1, z2, z3, z4 = torch.split(z_c, self.tch_add, dim=1)
    z1, z2, z3, z4 = z1.contiguous(), z2.contiguous(), z3.contiguous(), z4.contiguous()
    out0 = self.decFusion(x)
    out1 = self.dec1(out0, z1)
    out2 = self.dec2(out1, z2)
    out3 = self.dec3(out2, z3)
    out4 = self.dec4(out3, z4)
    out = self.decA5(out4)
